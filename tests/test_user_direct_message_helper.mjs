...
const helper = path.join(testDir, '..', 'scripts', 'send-feishu-user-direct-message');
...
async function fakeSecurityPath(t) {
  const directory = await mkdtemp(path.join(tmpdir(), 'feishu-user-direct-message-security-'));
...
function addDirectoryRoutes(request, response, url) {
  ...
  if (url.pathname === '/open-apis/contact/v3/scopes') {
    response.end(JSON.stringify({code: 0, data: {user_ids: ['user-target']}}));
    return true;
  }
  if (url.pathname === '/open-apis/contact/v3/users/batch') {
    response.end(JSON.stringify({code: 0, data: {items: [{user_id: 'user-target', open_id: 'open-target', name: '张超煜', email: 'target@example.com', status: {is_resigned: false}}]}}));
    return true;
  }
  return false;
}

test('sends one exact approved text message with Craig user authorization', async (t) => {
  const requests = [];
  const server = createServer(async (request, response) => {
    const url = new URL(request.url, 'http://127.0.0.1');
    const body = await readJson(request);
    requests.push({method: request.method, pathname: url.pathname, query: [...url.searchParams.entries()], authorization: request.headers.authorization, body});
    response.setHeader('Content-Type', 'application/json');
    if (addDirectoryRoutes(request, response, url)) return;
  ...
  const result = await run(['--recipient-name', '张超煜', '--text', '你好', '--confirmation', 'send_user_direct_message'], {
    ...process.env,
  ...
  const messageRequest = requests.find((item) => item.pathname === '/open-apis/im/v1/messages');
  ...
  assert.equal(requests.filter((item) => item.pathname === '/open-apis/im/v1/messages').length, 1);
});

test('rejects missing confirmation and unsafe text before reading credentials', async () => {
  for (const args of [
    ['--recipient-name', '张超煜', '--text', '你好'],
    ['--recipient-name', '张超煜', '--text', 'x'.repeat(2001), '--confirmation', 'send_user_direct_message'],
  ]) {
    const result = await run(args, process.env);
    assert.notEqual(result.code, 0);
    assert.match(result.stderr, /confirmation must equal send_user_direct_message|text must contain between 1 and 2000 characters/);
  }
});
...
test('does not send when exact name lookup is ambiguous', async (t) => {
  ...
  const result = await run(['--recipient-name', '张超煜', '--text', '你好', '--confirmation', 'send_user_direct_message'], {...process.env, PATH: `${securityPath}:${process.env.PATH}`, LARK_DOMAIN: `http://127.0.0.1:${server.address().port}`});

  assert.notEqual(result.code, 0);
  assert.equal(sent, false);
  assert.equal(result.stderr, 'FEISHU_USER_DIRECT_MESSAGE_ERROR stage=recipient_resolution feishu_code=unknown http_status=unknown\n');
});

test('scrubs a rejected official user-identity send', async (t) => {
  const server = createServer(async (request, response) => {
    const url = new URL(request.url, 'http://127.0.0.1');
    await readJson(request);
    response.setHeader('Content-Type', 'application/json');
    ...
    if (url.pathname === '/open-apis/im/v1/messages') {
      ...
      return response.end(JSON.stringify({code: 230027, msg: 'do not disclose this backend response'}));
    }
    response.statusCode = 404;
    response.end(JSON.stringify({code: 404}));
  });
  ...
  assert.notEqual(result.code, 0);
  assert.equal(result.stderr, 'FEISHU_USER_DIRECT_MESSAGE_ERROR stage=message_create feishu_code=230027 http_status=400\n');
});
